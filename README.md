...
| Topic | Assumption |
|-------|------------|
| Borrower unique id | Represented as `libraryId` (string), supplied by the API client on registration — not auto-generated by the system. |
...
| Topic | Assumption |
|-------|------------|
| Duplicate borrower | Registering an existing `libraryId` is rejected (`409`). |
...
| Extra: file import | An optional `POST /api/books/from-file` imports books from `.txt`/`.dat` (`id\|isbn\|title\|author`). Not required by the brief; added for convenience. |
...
### Borrow / return (on behalf of a borrower)

| Topic | Assumption |
|-------|------------|
| Required inputs | Both operations require `libraryId` (borrower) and `bookId` (`book_copy.id`). |
| Borrower must exist | Unknown `libraryId` → `404`. |
...
| Return ownership | Return is allowed only if the copy is `BORROWED` **by that same** `libraryId`; otherwise `409`. |
...
| Topic | Assumption |
|-------|------------|
| Style | RESTful JSON over HTTP; Spring Boot 3 + JPA. |
...
| Environments | `dev` / `prod` Spring profiles; prod DB credentials via environment variables. |
